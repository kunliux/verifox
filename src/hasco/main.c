...
**  $ cbmc --function addSpecRNE adder.c
**
** Note that this implementation does contain bugs in some modes.
** Contact the author if you can't find them :-)
*/
...
void main () {
  ...
  unpackedFloat uf = unpack_float(f);
  unpackedFloat ug = unpack_float(g);
  __CPROVER_assume((!uf.nan && !uf.zero && !uf.inf && !uf.subnormal));
  __CPROVER_assume((!ug.nan && !ug.zero && !ug.inf && !ug.subnormal));
