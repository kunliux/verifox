...
hasco_parseoptionst::hasco_parseoptionst(int argc, const char **argv):
        ...
        ui_message_handler(cmdline, std::string("Symex ") + CBMC_VERSION)
{
  //print_banner();
  /* std::cout << "HASCO 0.1\n";
   std::cout << "Hardware/Software Co-verification Tool\n";
   std::cout << "(Symbolic Execution with Trace Partitioning)\n";*/
}
void hasco_parseoptionst::print_banner()
{
  std::cout << "####################################################################" << std::endl;
  std::cout << "              VerifOx " << VERIFOX_VERSION << " Copyright (C) 2015"<< std::endl;
  std::cout << "      Saurabh Joshi, Rajdeep Mukherjee and Daniel Kroening" << std::endl;
...
void hasco_parseoptionst::eval_verbosity()
{
...
void hasco_parseoptionst::get_command_line_options(optionst &options)
{
  ...
  if(cmdline.isset("smt2"))
  {
    ...
    if(cmdline.isset("boolector"))
      ...
      options.set_option("cvc3",true);
    else if(cmdline.isset("cvc4"))
      options.set_option("cvc4",true);
    else if(cmdline.isset("opensmt"))
      options.set_option("opensmt",true);
...
int hasco_parseoptionst::doit()
{
  ...
  {
    const namespacet ns(symbol_table);
    path_searcht path_search(ns,options);

    path_search.set_message_handler(get_message_handler());

    if(cmdline.isset("depth"))
      path_search.depth_limit=unsafe_string2unsigned(cmdline.get_value("depth"));

    if(cmdline.isset("context-bound"))
      path_search.context_bound=unsafe_string2unsigned(cmdline.get_value("context-bound"));

    if(cmdline.isset("unwind"))
      path_search.unwind_limit=unsafe_string2unsigned(cmdline.get_value("unwind"));
...
bool hasco_parseoptionst::set_properties(goto_functionst &goto_functions)
{
  try
  {
...
bool hasco_parseoptionst::get_goto_program(
...
bool hasco_parseoptionst::process_goto_program(
        const optionst &options,
        goto_functionst &goto_functions)
{
  try
...
void hasco_parseoptionst::report_properties(
...
void hasco_parseoptionst::report_success()
{
...
void hasco_parseoptionst::show_counterexample(
...
void hasco_parseoptionst::report_failure()
{
...
void hasco_parseoptionst::help()
{
  std::cout <<
            "\n"
            "* *     VerifOx " VERIFOX_VERSION " - Copyright (C) 2015 ";
  ...
  std::cout <<
            "* *    Saurabh Joshi, Rajdeep Mukherjee and Daniel Kroening * *\n"
            ...
            << configt::this_operating_system() << ")\n"
                                                   ...
                                                   "\n"
                                                   "Solver options:\n"
                                                   "--smt2                        Use SMT2 format\n"
                                                   "--z3                          Use Z3 as SMT solver\n"
                                                   "Other options:\n"
